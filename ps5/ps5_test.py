...
class ProblemSet5(unittest.TestCase):
    ...
    def test7FilterStories(self):
        ...
        ta = NewsStory('1', "asfd New York City asfdasdfasdf", '', '', datetime.now())
        tb = NewsStory('2', "asdfasfd new york city! asfdasdfasdf", '', '', datetime.now())
        tnoa = NewsStory('3', "something somethingnew york city", '', '', datetime.now())
        tnob = NewsStory('4', "something something new york cities", '', '', datetime.now())

        st = DescriptionTrigger("New York City")
        a = NewsStory('5', '', "asfd New York City asfdasdfasdf", '', datetime.now())
        b = NewsStory('6', '', "asdfasfd new york city! asfdasdfasdf", '', datetime.now())
        noa = NewsStory('7', '', "something somethingnew york city", '', datetime.now())
        nob = NewsStory('8', '', "something something new york cities", '', datetime.now())

        triggers = [tt, st, self.tt, self.ft]
        stories = [ta,tb,tnoa,tnob,a, b, noa, nob]
        filtered_stories = filter_stories(stories, triggers)
        for story in stories:
            print(story.get_guid())
